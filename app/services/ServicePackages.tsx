    ...
    <section className="py-20 bg-gradient-to-br from-[#1C474A] to-[#0A2F33] relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-0 left-0 w-full h-full">
          ...
          <div className="absolute bottom-20 left-20 w-80 h-80 bg-white rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }}></div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className={`text-center mb-16 transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
          ...
          <p className="text-xl text-white/80 max-w-3xl mx-auto leading-relaxed">
            نقدم باقات متنوعة تناسب جميع الاحتياجات والميزانيات مع ضمان أعلى معايير الجودة
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
          {packages.map((pkg, index) => (
            ...
            >
              ...
              {/* البطاقة */}
              <div className={`relative bg-white/10 backdrop-blur-sm rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 border-2 ${
                selectedPackage === pkg.id ? 'border-[#CBC1B8] shadow-2xl' : 'border-white/20'
              } ${pkg.popular ? 'border-[#CBC1B8]/50' : ''} hover:bg-white/20`}>
                
                {/* الهيدر */}
                <div className={`relative p-8 bg-gradient-to-r ${pkg.color} text-white`}>
                  <div className="absolute inset-0 bg-black/10"></div>
                  <div className="relative z-10">
                    <h3 className="text-2xl font-['Cairo'] font-bold mb-2">{pkg.name}</h3>
        ...
        {/* قسم المقارنة */}
        <div className={`bg-white/10 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-white/20 transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`} style={{ animationDelay: '600ms' }}>
          <div className="text-center mb-8">
            <h3 className="text-3xl font-['Cairo'] font-bold text-white mb-4">
              مقارنة سريعة بين الباقات
            </h3>
            <p className="text-white/80">اختر الباقة التي تناسب احتياجاتك وميزانيتك</p>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b-2 border-white/20">
                  <th className="text-right py-4 px-6 font-semibold text-white">الميزة</th>
                  <th className="text-center py-4 px-6 font-semibold text-white">اقتصادية</th>
                  <th className="text-center py-4 px-6 font-semibold text-[#CBC1B8]">متوسطة</th>
                  <th className="text-center py-4 px-6 font-semibold text-white">فاخرة</th>
                </tr>
              </thead>
              <tbody>
                {[
                  { feature: 'التصميم ثلاثي الأبعاد', basic: false, standard: true, premium: true },
                  { feature: 'الإشراف المستمر', basic: false, standard: true, premium: true },
                  { feature: 'الأثاث المخصص', basic: false, standard: true, premium: true },
                  { feature: 'أنظمة المنزل الذكي', basic: false, standard: false, premium: true },
                  { feature: 'الصيانة الدورية', basic: false, standard: false, premium: true }
                ].map((row, index) => (
        ...
        {/* قسم النصائح */}
        <div className={`text-center mt-16 transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`} style={{ animationDelay: '800ms' }}>
