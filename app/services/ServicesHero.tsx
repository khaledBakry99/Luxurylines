      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 opacity-10">
          <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
            <defs>
              <pattern id="grid-services" width="4" height="4" patternUnits="userSpaceOnUse">
                <path d="M 4 0 L 0 0 0 4" fill="none" stroke="#CBC1B8" strokeWidth="0.2"/>
              </pattern>
            </defs>
            <rect width="100%" height="100%" fill="url(#grid-services)" className="animate-pulse"/>
          </svg>
        </div>
      </div>
      ...
      <div className="relative z-10 text-center text-white px-6 max-w-7xl mx-auto">
        <div className={`transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
          ...
          <div className="inline-flex items-center px-6 py-3 bg-[#CBC1B8]/20 backdrop-blur-sm border border-[#CBC1B8]/30 rounded-full mb-8 animate-fade-in">
            ...
            <span className="text-[#CBC1B8] font-semibold">خدماتنا المتميزة</span>
          </div>
          ...
          <p className="text-xl md:text-2xl mb-12 max-w-4xl mx-auto leading-relaxed text-white/90">
            نقدم مجموعة شاملة من الخدمات المتخصصة في التصميم والتشطيب وتوريد الأساس، مع فريق من الخبراء المبدعين 
            لتحويل مساحاتك إلى تحف فنية تعكس شخصيتك وأسلوب حياتك
          </p>

          {/* الخدمات الأربعة الرئيسية */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
            {[
              { name: 'تصميم داخلي', icon: 'ri-home-4-line', color: 'from-[#0A1F21] to-[#1C474A]' },
              { name: 'تصميم خارجي', icon: 'ri-building-line', color: 'from-[#0F3A3E] to-[#0A2F33]' },
              { name: 'تشطيب', icon: 'ri-paint-brush-line', color: 'from-[#1C474A] to-[#0A1F21]' },
              { name: 'توريد أساس', icon: 'ri-truck-line', color: 'from-[#0A2F33] to-[#1C474A]' }
            ].map((service, index) => (
              <div
                key={index}
              ...
              >
                <div className={`w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-gradient-to-br ${service.color} rounded-xl group-hover:scale-110 transition-all duration-300 shadow-lg`}>
